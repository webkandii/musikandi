		        ...
		        while ($data = $get_data->fetch()) {
		            ...
		            if(logged_in() == true) {
		            	...
		            	if($_SESSION['permissions'] == 'admin' || $owned_by == $_SESSION['id']) {
		            		$actions .= '<a href="edit.php?r=' . $id . '&table=' . $table . '&return=' . $return_url . '" class="edit-record"></a> <a href="delete.php?r=' . $id . '&table=' . $table . '&return=' . $return_url . '" onclick="return confirm("Are you sure you want to delete this record (ID ' . $id . ') from ' . $table . '?")" class="delete-record"></a>';
		            	}
		            }
...
row;
	...
	function validate_user($id, $first_name, $last_name, $email, $permissions, $ab_count, $ab_agents, $ab_worldwide, $ab_promoters, $ab_digital, $ab_print, $ab_broadcast, $ab_venues, $ab_london, $ab_festivals, $ab_nonukfests, $ab_tech) {
        ...
        $_SESSION['ab_national'] = $ab_national;
        $_SESSION['ab_nonukfests'] = $ab_nonukfests;
        $_SESSION['ab_tech'] = $ab_tech;
	}
